  ...
  try {
    ...
    if (!tokenOwner) {
      return next(
        createError(
          401,
          "This user already deleted! The token is not valid anymore"
        )
      );
    }
    ...
    if (
      tokenOwner.passChangeDate &&
      tokenOwner.passChangeDate.getTime() / 1000 > decoded.iat
    ) {
      return next(
        createError(
          401,
...
export const restrictTo = (...allowedRoles) => {
  ...
  return (req, res, next) => {
    if (!allowedRoles.includes(req.user.role)) {
      //'user'
      return next(
        createError(403, "You have no permission to do this operation.")
      );
    }
